...
//' Compute the pdf for an inverse Gaussian distribution
//'
//' @param tau points where to evaluate the pdf
...
//' Compute the inverse cdf (survival function) for an inverse Gausssian 
//' distribution
//'
//' @param tau points where to evaluate the inverse cdf
//' @param mu location parameter
//' @param lambda shape parameter
//' @param delta offset parameter
//' @param log should the results be returned on the log scale?
...
//' @param mu matrix of size (n x d_1) containing the drift parameters
//' corresponding to every individual (one for every possible response)
//' @param b matrix of size (n x d_1) containing the boundary parameters
//' corresponding to every individual (one for every possible response)
//' @param delta vector of size n containing the offset parameter corresponding
//' to every individual
//' @param cens vector of size n containing censoring indicators (1 censored, 0
//' non censored)
//' @param D (n x 2) matrix with input tone and response category on the columns
//' @param log should the results be returned on the log scale?
//' @export
// [[Rcpp::export]]
double  log_likelihood_cpp(arma::vec tau, arma::mat mu, arma::mat b, 
...
//'
//' @param x input vector of categories
...
//' @return vector with the row sums (on the log scale)
//' @export
// [[Rcpp::export]]
...
//' Given the cluster memberships over time, count the transition matrix
//'
...
//' @param delta_dat vector of size n containing the offset parameter 
//' corresponding to every individual
//' @param b_dat matrix of size (n x 4) containing the boundary parameters 
//' corresponding to every individual (one for every possible response)
//' @param B_beta_dat 
//' @param mu_dat_old matrix of size (n x 4) containing the drift parameters 
//' corresponding to every individual (one for every possible response)
...
//' @param time vector denoting the time (block)
...
Rcpp::List sample_reff_mu(arma::vec tau, arma::umat D, arma::uvec cens, 
  ...
  for (unsigned int i = 1; i <= n_ind; i++){
    for (unsigned int k = 0; k < J; k++){
      ...
      logpost_prop = log_likelihood_cpp(tau.elem(idx_it), 
                                        mu_dat_prop, 
                                        b_dat.rows(idx_it), 
                                        ...
                                        D.rows(idx_it), true) + 
                                          logpr_prop;
      logpost_old = log_likelihood_cpp(tau.elem(idx_it), 
...
//' corresponding to every individual (one for every possible response)
//' @param B_beta_dat 
//' @param b_dat_old matrix of size (n x 4) containing the boundary parameters 
//' corresponding to every individual (one for every possible response)
//' @param B expansion of the blocks on the spline basis
//' @param P smoothness inducing matrix (first order differences)
//' @param ind vector denoting the individuals
